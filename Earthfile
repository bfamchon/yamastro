...
build:
    ...
    # NOTE: change this if you set a custom cacheDir in your astro config
    LET astro_cacheDir="./node_modules/.astro"
    CACHE $astro_cacheDir

    COPY . .

    # Uncomment if you wish to disable telemetry. Learn more at: https://docs.astro.build/en/reference/cli-reference/#astro-telemetry
    #ENV ASTRO_TELEMETRY_DISABLED=1
    ENV YAMA_ENV=$yama_env

    # Assumes you have a "build" script in your package.json to run `astro build`. Edit as needed.
    RUN yarn run build
...
# Functions (User-defined commands) to store the project metadata as artifacts
SAVE_DEPENDENCIES:
    FUNCTION
...
# Run `astro dev` dev server with $PWD mounted. Run locally with `earthly +dev`.
